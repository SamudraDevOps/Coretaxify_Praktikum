...
function numberToTerbilang(n) {
    ...
    if (n < 100) return numberToTerbilang(Math.floor(n / 10)) + " puluh" + (n % 10 !== 0 ? " " + numberToTerbilang(n % 10) : "");
    if (n < 200) return "seratus" + (n - 100 !== 0 ? " " + numberToTerbilang(n - 100) : "");
    if (n < 1000) return numberToTerbilang(Math.floor(n / 100)) + " ratus" + (n % 100 !== 0 ? " " + numberToTerbilang(n % 100) : "");
    ...
    if (n < 1000000000000) return numberToTerbilang(Math.floor(n / 1000000000)) + " miliar" + (n % 1000000000 !== 0 ? " " + numberToTerbilang(n % 1000000000) : "");
    if (n < 1000000000000000) return numberToTerbilang(Math.floor(n / 1000000000000)) + " triliun" + (n % 1000000000000 !== 0 ? " " + numberToTerbilang(n % 1000000000000) : "");
...
const SelfBilling = () => {
    ...
    const steps = [
        {
            ...
            description: (
                <p>
                    Pastikan identitas wajib pajak Anda sudah benar dan lengkap sebelum melanjutkan ke tahap berikutnya.
                </p>
            ),
        },
        {
            ...
            description: (
                <>
                    <p><strong>KAP</strong> adalah kode yang mengidentifikasi jenis pajak yang harus dibayar oleh wajib pajak. Setiap jenis pajak, seperti Pajak Penghasilan (PPh), Pajak Pertambahan Nilai (PPN), dan pajak lainnya, memiliki KAP yang berbeda.</p>
                    <p className="mt-2"><strong>KJS</strong> adalah kode yang menunjukkan jenis setoran pajak yang dilakukan oleh wajib pajak. Kode ini mencerminkan cara dan tujuan setoran, seperti pembayaran pajak terutang, setoran denda, atau setoran untuk angsuran.</p>
                </>
            ),
            fields: [
                {
        ...
        },
        {
            ...
            description: (
                <p>
                    Anda telah berhasil membuat kode billing. Silakan unduh kode pembayaran untuk proses selanjutnya.
                </p>
            ),
        },
    ];
    ...
    const MonthYearPicker = ({ label, selected, onChange, showYearOnly = false }) => {
        return (
            <div>
        ...
        <div className="bg-white p-6 rounded-md shadow-sm max-w-5xl mx-auto">
            ...
            <div className="relative flex justify-between items-center mb-10">
                {steps.map((item, index) => (
                    <div key={item.id} className="flex-1 flex flex-col items-center relative z-10">
            ...
            <div className="space-y-6">
                <div>
                    ...
                    {step === 2 && (
                        <div className="text-gray-600 text-sm mt-2">{steps[1].description}</div>
                    )}
                </div>
                ...
                {step === 1 && (
                    <div className="text-sm space-y-4">
                        ...
                        <Input label="Nama Wajib Pajak" value={name} onChange={setName} required />
                        <TextArea label="Alamat Wajib Pajak" value={address} onChange={setAddress} required />
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="text-sm space-y-4">
                        <p className="text-gray-700">
                            Harap pastikan dengan cermat bahwa semua informasi terkait identitas wajib pajak Anda telah diperiksa dan dikonfirmasi. Ini mencakup nama lengkap Anda, alamat tempat tinggal atau domisili,
                            serta Nomor Pokok Wajib Pajak (NPWP) Anda. Selain itu, pastikan bahwa Kode Akun Pajak (KAP) dan Kode Jenis Setoran (KJS) yang Anda gunakan sudah benar dan sesuai. Ketelitian dalam memverifikasi kesesuaian informasi ini sangat penting, terutama dalam konteks proses pembentukan kode penagihan, untuk menghindari kesalahan yang dapat menghambat proses administrasi pajak dan potensi masalah di masa depan
                        </p>
                        <div className="bg-gray-50 p-4 border rounded">
                            <div className="grid grid-cols-2 gap-y-2 text-sm">
                                ...
                                <div className="font-semibold text-gray-700">Nama Wajib Pajak</div>
                                <div>: {name}</div>

                                <div className="font-semibold text-gray-700">Alamat Wajib Pajak</div>
                                <div>: {address}</div>

                                <div className="font-semibold text-gray-700">KAP - KJS</div>
                                <div>: {kapKjs}</div>

                                <div className="font-semibold text-gray-700">Periode dan Tahun Pajak</div>
                                <div>: {selectedMonth?.toLocaleString('default', { month: 'long' })} {selectedYear?.getFullYear()}</div>
                            </div>
                        </div>
                        ...
                        <div className="mt-4 space-y-4">
                            <div>
                                <label className="block text-gray-700 font-medium mb-1" htmlFor="currency">
                                    Mata Uang
                                </label>
                                <select
                                    id="currency"
                            ...
                            <Input label="Nilai" value={nilaiTampil} onChange={handleNilaiChange} required />
                            <Input label="Terbilang" value={terbilang} onChange={() => { }} required readOnly />
                            <TextArea label="Keterangan" value={keterangan} onChange={setKeterangan} />
                        </div>
                    </div>
                )}
