...
export default function UniversalLabaRugi({ jenisPerusahaan, bagian = 'A' }) {
    ...
    try {
        const { initialRows, config, kodeKoreksiOptions } = LabaRugiFactory.createComponent(jenisPerusahaan, bagian);
        ...
        if (!rows || rows.length === 0) {
            return (
                <div style={{ padding: '20px', textAlign: 'center', color: '#666' }}>
                    <p>Tidak ada data untuk {jenisPerusahaan} - {bagian}</p>
                </div>
            );
        }
        ...
        console.error('Error rendering UniversalLabaRugi:', error);
    }
}