...
export function hitungSubtotal4300(rows) {
  ...
  const tambah = ["4026", "4101", "4071", "4001"];
...
export function hitungTotal4800(rows) {
  ...
  kolom.forEach((key) => {
    const labaUsaha = toNumber(rows.find((r) => r.kodeAkun == "4300")?.[key]);
    const pendapatanLain =
      toNumber(rows.find((r) => r.kodeAkun == "4153")?.[key]) +
      toNumber(rows.find((r) => r.kodeAkun == "4199")?.[key]);

    const bebanKoreksi = toNumber(rows.find((r) => r.kodeAkun == "5324")?.[key]);
    const bebanUsaha = toNumber(rows.find((r) => r.kodeAkun == "5400")?.[key]);
    const nonUsaha =
      toNumber(rows.find((r) => r.kodeAkun == "4600")?.[key]) -
      toNumber(rows.find((r) => r.kodeAkun == "5500")?.[key]);

    target[key] = labaUsaha + pendapatanLain - bebanKoreksi - bebanUsaha + nonUsaha;
  });
