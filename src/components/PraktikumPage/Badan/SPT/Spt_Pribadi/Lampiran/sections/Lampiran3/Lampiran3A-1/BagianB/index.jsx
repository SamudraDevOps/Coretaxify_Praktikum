...
const INITIAL_RIGHT_ROWS = [
  ...
  },
  {
    ...
    type: "subtotal",
    level: 0,
    keterangan: "TOTAL LIABILITAS & EKUITAS",
    nilaiKomersial: 0,
  },
];
  ...
  const leftRowsWithCalculation = useMemo(() => {
    return leftRows.map((row) => {
      if (row.id === "1700") {
        ...
        const nilaiKomersial = calculateWithDeductions(addIds, subtractIds, leftRows);

        return { ...row, nilaiKomersial };
      }
  ...
  const rightRowsWithCalculation = useMemo(() => {
    return rightRows.map((row) => {
      ...
      if (row.id === "2999") {
        ...
        const nilaiKomersial = liabilitasIds.reduce((sum, id) => {
          const liabRow = rightRows.find((r) => r.id === id);
          return sum + toNum(liabRow?.nilaiKomersial);
        }, 0);
      ...
      if (row.id === "3299") {
        ...
        const nilaiKomersial = ekuitasIds.reduce((sum, id) => {
          const ekRow = rightRows.find((r) => r.id === id);
          return sum + toNum(ekRow?.nilaiKomersial);
        }, 0);
      ...
      if (row.id === "3300") {
        ...
        const nilaiKomersial = calculatedLiab + calculatedEkuitas;
        return { ...row, nilaiKomersial };
      }
