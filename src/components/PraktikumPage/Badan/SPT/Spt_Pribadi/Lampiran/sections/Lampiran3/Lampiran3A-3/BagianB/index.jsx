...
//  Digunakan HANYA untuk menyamakan tinggi tabel
const createFillerRows = (count) =>
...
export default function BagianB() {
  ...
  const maxRows = Math.max(leftRowsWithCalculation.length, rightRowsWithCalculation.length);

  const normalizedLeftRows = [
    ...leftRowsWithCalculation,
    ...createFillerRows(maxRows - leftRowsWithCalculation.length),
  ];

  const normalizedRightRows = [
    ...rightRowsWithCalculation,
    ...createFillerRows(maxRows - rightRowsWithCalculation.length),
  ];
  ...
  const createColumns = (side) => [
    ...
    },
    {
      ...
      align: "center",
      render: (row) => {
        if (row.type === "header" || row.type === "filler") return "";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 items-start overflow-visible">
        {/* Tabel Kiri: ASET */}
        <div>
          ...
          <GlobalTable
            columnGroups={createColumns("left")}
            data={normalizedLeftRows}
            page={1}
            pageSize={9999}
            ...
            // stickyHeader
            rowClassName={(row) => {
              if (row.type === "filler") return "bg-transparent border-none ";
        ...
        <div>
          ...
          <GlobalTable
            columnGroups={createColumns("right")}
            data={normalizedRightRows}
            page={1}
            pageSize={9999}
