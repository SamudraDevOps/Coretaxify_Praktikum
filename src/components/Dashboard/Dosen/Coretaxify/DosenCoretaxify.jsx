...
const DosenCoretaxify = () => {
  ...
  const apiUrl = useMemo(() => {
    ...
    return `${RoutesApi.url}api/admin/self-assignments?${params}`;
  }, [currentPage, search]);
  ...
  const { isLoading: isLoadingTasks, data: dataTasks } = useQuery({
    queryKey: ["tasks"],
    queryFn: async () => {
      const { data } = await axios.get(`${RoutesApi.url}api/lecturer/contract-tasks`, {
        headers: {
          Authorization: `Bearer ${cookies.token}`,
        },
      });
  ...
  const createMutation = useMutation({
    mutationFn: async (formData) => {
      ...
      const response = await axios.post(
        `${RoutesApi.url}api/admin/self-assignments`,
        submitData,
        {
          headers: {
          ...
          params: {
            intent: IntentEnum.API_STORE_SELF_ASSIGNMENT,
          },
        }
      );
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, formData }) => {
      ...
      const response = await axios.post(
        `${RoutesApi.url}api/admin/self-assignments/${id}`,
        submitData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
