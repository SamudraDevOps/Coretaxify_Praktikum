...
import EditKodeTransaksi from "./Edit";
import TambahKodeTransaksi from "./Create";
...
const KodeTransaksi = () => {
  ...
  const [url, setUrl] = useState(`${RoutesApi.apiUrl}kode-transaksi`);
  const { toast } = useToast();

  const { isLoading, isError, data, error, refetch } = useQuery({
    queryKey: ["kode_transaksi", url, currentPage],
    queryFn: async () => {
  ...
  const mutation = useMutation({
    mutationFn: async ({id}) => {
      ...
      const deleteEndpoint = `${RoutesApi.apiUrl}kode-transaksi/${id}`;
    ...
    onSuccess: () => {
      Swal.fire({
        title: "Berhasil!",
        text: "Kode Transaksi Berhasil Dihapus!",
        icon: "success",
      ...
      <div className="search-add-container">
        <div className="flex items-center">
          <input
            ...
            placeholder="Cari Data Kode Transaksi"
            ...
            {!isLoading &&
              data?.data?.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="whitespace-normal break-words max-w-[200px]">{item.nama_transaksi}</td>
