...
import EditKapKjs from "./Edit";
import TambahKapKjs from "./Create";
...
const KapKjs = () => {
  ...
  const [url, setUrl] = useState(`${RoutesApi.apiUrl}kap-kjs`);
  const { toast } = useToast();

  const { isLoading, isError, data, error, refetch } = useQuery({
    queryKey: ["kap_kjs", url, currentPage],
    queryFn: async () => {
  ...
  const mutation = useMutation({
    ...
    //   const deleteEndpoint = RoutesApi.admin.universities.destroy(id);
      const deleteEndpoint = `${RoutesApi.apiUrl}kap-kjs/${id}`;
            ...
            {!isLoading &&
              data?.data?.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="whitespace-normal break-words max-w-[200px]">{item.kode}</td>
                  <td className="whitespace-normal break-words max-w-[200px]">{item.ket_1}</td>
                  <td className="whitespace-normal break-words max-w-[200px]">{item.ket_2}</td>
                  <td>
                    <button
                    ...
                    <button
                      ...
                      onClick={() => {
                        Swal.fire({
                          title: "Hapus KAP-KJS?",
                          text: "KAP-KJS akan dihapus secara permanen!",
                          icon: "warning",
