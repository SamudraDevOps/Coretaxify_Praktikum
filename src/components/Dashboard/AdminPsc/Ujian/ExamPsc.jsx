...
const ExamPsc = () => {
        ...
        mutationFn: async ({ id, action, formData: examFormData }) => {
            ...
            if (action === "create") {
                ...
                if (examFormData.start_period) {
                    formDataObj.append("start_period", formatDateForBackend(examFormData.start_period));
                }

                if (examFormData.end_period) {
                    formDataObj.append("end_period", formatDateForBackend(examFormData.end_period));
                }

                return await axios.post(
                    RoutesApi.psc.exams.store().url,
                    formDataObj,
                    {
                        headers: {
                ...
                if (examFormData.task_id) formDataObj.append("task_id", examFormData.task_id);
                if (examFormData.duration) formDataObj.append("duration", examFormData.duration);
                if (examFormData.supporting_file) formDataObj.append("supporting_file", examFormData.supporting_file);
                if (examFormData.start_period) formDataObj.append("start_period", formatDateForBackend(examFormData.start_period));
                if (examFormData.end_period) formDataObj.append("end_period", formatDateForBackend(examFormData.end_period));
    ...
    const handleViewMembers = (id, name) => {
    ...
    const currentActiveItems = activeExams.slice(indexOfFirstActiveItem, indexOfLastActiveItem);
    ...
    const currentOtherItems = otherExams.slice(indexOfFirstOtherItem, indexOfLastOtherItem);
    ...
    return (
        <div className="ujian-container">
            ...
            {activeExams.length > 0 && (
                <div className="mb-8">
                    <h3 className="text-xl font-semibold mb-4 text-gray-700">Ujian Aktif</h3>
                                ...
                                {currentActiveItems.length > 0 ? (
                                    currentActiveItems.map((exam, index) => (
                                        <tr key={exam.id}>
                                            ...
                                            <td>{exam.supporting_file || "No file"}</td>
                        ...
                        {activeExams.length > itemsPerPage && (
                            <div className="pagination-container sticky">
                                ...
                                <div className="pagination">
                                    ...
                                    <button
                                        className={`page-item ${currentPage === Math.ceil(activeExams.length / itemsPerPage) ? "disabled" : ""}`}
                                        onClick={() => paginate(currentPage + 1)}
                                        disabled={currentPage === Math.ceil(activeExams.length / itemsPerPage)}
                                    >
                                        &gt;
                                    </button>
                                </div>
            ...
            {otherExams.length > 0 && (
                <div>
                    <h3 className="text-xl font-semibold mb-4 text-gray-700">Ujian Lainnya</h3>
                    <div className="table-container">
                        <table>
                            <thead>
                                <tr>
                        ...
                        {otherExams.length > itemsPerPage && (
                            <div className="pagination-container sticky">
                                ...
                                <div className="pagination">
                                    ...
                                    <button
                                        className={`page-item ${currentPage === Math.ceil(otherExams.length / itemsPerPage) ? "disabled" : ""}`}
                                        onClick={() => paginate(currentPage + 1)}
                                        disabled={currentPage === Math.ceil(otherExams.length / itemsPerPage)}
                                    >
                                        &gt;
                                    </button>
                                </div>
